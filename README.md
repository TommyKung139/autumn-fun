...
└── scripts/
    ├── build_employees.py / build_table_layout.py / build_combined_layout.py
    │                        資料是怎麼從 Excel 整理出來的（供之後更新資料用，部署網站不需要它）
    └── seed_firestore.js   把 data/employees.json 匯入 Firestore 的一次性腳本（見下方說明）
```
