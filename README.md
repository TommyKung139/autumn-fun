...
給同仁查詢自己（或其他同仁）8/29 第二梯秋郊行程的內部小網站：輸入員工編號或姓名即可看到集合車站、午宴桌次、下午／晚上行程、遊覽車、高鐵車廂座位與發車時間（如已排定）與攜眷資訊；另有「行程介紹」頁面（當日完整行程時間表、島語午宴地點、座位圖、餐檯位置、洲際棒球場位置說明）；梯長／福委另有一個「資料編輯後台」（`/edit.html`）可即時調整同仁的行程資料，不需要重新部署網站。
...
```
├── index.html      首頁：輸入員工編號查詢行程（含高鐵時刻）
├── info.html       行程介紹：當日完整行程時間表、島語午宴地點、座位圖、洲際棒球場位置
├── charts.html      分車座位表：高鐵車廂座位圖、遊覽車分車表、島語分桌表（可標示自己的位置）
...
│   ├── employee-create.js 編輯後台新增同仁用（需 editor token）
│   ├── charts.js         回傳高鐵座位表／分車表／分桌表（即時組出＋快取）
...
│   ├── charts.js     從同仁資料即時組出高鐵座位表／分車表／分桌表
...
把梯長／福委當天使用的三張編排表原樣放上網站，同仁可以看到自己旁邊坐誰、同車有誰：

- **高鐵座位表**：**一個車次收合成一區**（例如「684 次」底下就含 2 車／3 車／4 車），展開後每節車廂各畫成一張座位圖（排數 ＋ A/B/C ｜ D/E，中間留走道），底色代表處別。眷屬座位與未指名的預留席不在此表中（系統只存同仁本人的座位，眷屬座位請看實體車票）。同一個座位被指派給兩個人時會用紅框標出。
- **遊覽車分車表**：分車表一（上午去程）與分車表二（下午行程）每一台車的完整名單，眷屬與同仁同車。尚未分車的同仁會列在表格上方。
- **島語分桌表**：A／B／C 區每一桌的名單，眷屬與同仁同桌，並連到行程介紹頁的平面圖。

收合單位是「一個車次／一台遊覽車／一桌」，預設全部收合，另有「全部展開」「全部收合」兩顆按鈕。
高鐵的部分刻意用車次而不是車廂當收合單位，因為同一個車次可能有好幾節車廂、上下車站也各不相同，
分太細反而要開很多次。
頁面上方輸入姓名可以把自己的位置標成黃色，含有你的那幾個區塊會自動展開並標上「★ 你在這裡」（眷屬用斜體顯示，也會一起亮起來）。
從「行程查詢」的個人卡片點「看完整車廂座位表 →」「看分車表 →」「看分桌表 →」也會自動帶入姓名並捲動到自己的位置。

這三張表**不是** Excel 的靜態快照，而是由 `lib/charts.js` 直接從目前系統中的同仁資料即時組出來的
（有設定 Firebase 就讀 Firestore，沒有就讀本機 JSON）。因此在後台改過的座位、車次、桌次會直接反映在這裡，
不需要重新匯出或重新部署。

因為組表需要讀取整個 employees collection（Firestore 是一位同仁一次讀取），`api/charts.js` 會把結果在
函式實例中快取一段時間，預設 **5 分鐘**，避免整梯同時開這一頁時把 Firestore 免費額度用完。可以用環境變數
`CHARTS_CACHE_SECONDS` 調整（數字越小越即時、讀取次數越多）。剛改完資料想立刻看到結果，可以按頁面上的
「立即重新讀取」，那會略過快取重新讀一次。

**隱私提醒**：這一頁會列出全梯同仁的姓名與座位／車次／桌次編排（但完全不含身分證字號、生日、電話）。和其他頁面一樣需要先輸入通關密碼才看得到，網址請勿外流。若日後不希望同仁互相看到，把 `charts.html` 的 nav 連結移除、或把 `api/charts.js` 的 `requireAuth` 換成 `requireEditor` 即可改成只有梯長／福委看得到。
...
行程查詢頁與編輯後台的搜尋框，員工編號和姓名都可以輸入：

- **員工編號**：大小寫都可以，前面有沒有加 `Z` 也都可以。這點是必要的——報名表匯出的資料裡有一筆同仁的員工編號是**小寫**（`z00039583`），而 Firestore 的文件 ID 有分大小寫，如果照著一般寫法輸入大寫就會查不到人。`lib/firestore.js` 的 `resolveEmployee()` 會依序嘗試原樣／全大寫／全小寫／去掉或補上開頭的 Z，找到為止；編輯儲存時也會寫回資料真正所在的那個 ID，不會生出重複記錄。
- **姓名**：可以只打姓氏或前幾個字（用的是前綴比對，所以打「林芝萱」也找得到登記為「林芝萱(Chih)」的同仁）。**同名時會列出所有符合的同仁讓你點選**，例如有兩位陳怡君（經策／數據營運部、數平／數位平台部）。
  姓名查詢在 Firestore 上用的是 `where('name','==')` 與前綴範圍查詢，兩者都是索引查詢，只會依實際符合的筆數計費，不會整個 collection 掃一遍。
...
- **新增同仁**：頁面上有「＋ 新增一位同仁」可以建立一筆全新資料（例如後來才補報名、或報名表漏掉的人）。只有**員工編號**與**姓名**必填，其餘欄位可先留空，建立完成後會直接跳到該員的編輯表單繼續補齊。員工編號重複時會擋下並提示改用搜尋編輯，避免覆蓋既有資料。
- 編輯既有資料時，**姓名、員工編號、梯次、眷屬明細**不開放修改（這些是報名表的原始身分資料，如需更正請直接調整來源 Excel 並重新執行 `scripts/build_employees.py` 與 `scripts/seed_firestore.js`）。新增同仁時姓名與員編當然可以填，因為那是資料建立的當下。
- 透過新增功能建立的同仁會**立即**出現在 `/charts.html` 的座位表／分車表／分桌表上（那三張表是即時從同仁資料組出來的），只要有填遊覽車／桌次／高鐵座位就會排進去。
...
3. 部署完成後，到 [Vercel Dashboard](https://vercel.com/dashboard) 找到這個專案 → **Settings → Environment Variables**，新增：
   ...
   - （選用）`CHARTS_CACHE_SECONDS`：分車座位表的快取秒數，預設 300
...
- 高鐵時間是依「行程說明」總表（`data/reference.json` 的 `day_schedule`）與每個人的車次／站別自動換算，去程與回程取的是時刻表的**不同端點**，這點很容易搞錯：
  - 報名表的去程欄位是「**上車**地點」，而所有去程車次的**終點都是台中**，所以個人站別對應的是**發車**時間，台中則是抵達時間。
  - 報名表的回程欄位是「**下車**地點」，而所有回程車次的**起點都是台中**，所以台中才是**發車**時間，個人站別對應的是**抵達**時間。
  （例如 684 次是 21:00 從台中發車、21:50 才抵達板橋。若把回程的下車站當成上車站去查，就會把抵達時間當成發車時間顯示。）
  頁面上兩段都會完整寫成「X 站 hh:mm 發車 → Y 站 hh:mm 抵達」，避免誤讀。車次未知且該站有多班車經過時，會列出所有可能班次並提示以現場公告為準。
- 若座位表登記的車次其實沒有停靠該同仁的站別（來源資料目前有 1 筆：回程左營被登記在 666 次，但左營是 663 次停靠），頁面會改用實際停靠該站的班次時間，並附註說明兩者不一致，請以實體車票為準。
