...
- **高鐵座位表**：依車次／車廂畫成座位圖（排數 ＋ A/B/C ｜ D/E，中間留走道），底色代表處別。眷屬座位與未指名的預留席不在此表中（系統只存同仁本人的座位，眷屬座位請看實體車票）。同一個座位被指派給兩個人時會用紅框標出。
- **遊覽車分車表**：分車表一（上午去程）與分車表二（下午行程）每一台車的完整名單，眷屬與同仁同車。尚未分車的同仁會列在表格上方。
- **島語分桌表**：A／B／C 區每一桌的名單，眷屬與同仁同桌，並連到行程介紹頁的平面圖。

每一個車廂／每一台車／每一桌都可以單獨展開收合，預設全部收合，另有「全部展開」「全部收合」兩顆按鈕。
