...
function buildHsr(employees, legField) {
  ...
  const cars = [...blocks.values()].map((b) => {
  ...
  // Group the carriages under their train, so the page collapses one section per
  // 車次 rather than one per carriage-and-boarding-station combination.
  const trains = new Map();
  for (const car of cars) {
    if (!trains.has(car.train_no)) {
      trains.set(car.train_no, { train_no: car.train_no, cars: [], stations: new Set(), seat_count: 0, conflicts: 0 });
    }
    const t = trains.get(car.train_no);
    t.cars.push(car);
    car.stations.forEach((s) => t.stations.add(s));
    t.seat_count += car.seats.length;
    t.conflicts += car.conflicts;
  }

  return [...trains.values()]
    .map((t) => {
      t.cars.sort((a, b) => a.car.localeCompare(b.car, undefined, { numeric: true }));
      return Object.assign(t, { stations: [...t.stations] });
    })
